...
Hagamos un modelo GLM Multinomial con penalizacion elástica usando glment. Probemos cuanto se demora con el 5% de los datos y usando Lasso (alpha=1)
...
Esto quiere decir que un modelo con todos los datos se puede demorar una media hora. Carguemos los datos que se usaron en el servidor para que sirvan de base para el resto de modelos.
...
```
```{r,include=FALSE}
#save(errorGLMTest_All,glm_test_time,file = "cache/glm_test_all.RData")
load("cache/glm_test_all.RData")
errorGLMTest_All
```

El modelo obtuvo una tasa de clasificación correcta del 88.6% lo que lo pone muy por debajo del SVM.